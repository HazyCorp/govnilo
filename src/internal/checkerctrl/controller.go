...
type Controller struct {
	l                  *zap.Logger
	registeredCheckers map[string]checker.Checker
	registeredSploits  map[checker.SploitID]checker.Sploit
	...
	sploitMetrics   map[checker.SploitID]*sploitMetrics
...
func New(in ControllerIn) *Controller {
	nameToChecker := make(map[string]checker.Checker, len(in.Checkers))
	for _, check := range in.Checkers {
		nameToChecker[check.ServiceName()] = check
	}

	idToSploit := make(map[checker.SploitID]checker.Sploit, len(in.Sploits))
	...
	return &Controller{
		...
		sploitMetrics:      make(map[checker.SploitID]*sploitMetrics),
	}
}
...
func (c *Controller) syncState(ctx context.Context) error {
	...
	for svcName, svc := range currentState.Services {
		...
		for sploitName, state := range svc.Sploits {
			sploitID := checker.SploitID{Service: svcName, Name: sploitName}
...
func (c *Controller) sploitMetricsFor(sploitID checker.SploitID) *sploitMetrics {
